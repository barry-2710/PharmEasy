     ...
     <?php include "navbar.php"; ?>
        <div class="main-content p-4" style="min-height:90vh;">
           ...
           <div class="products p-4 bg-light">
                ...
                $total_pages_sql = "SELECT COUNT(*) FROM products WHERE category='medicine'";
                $result = mysqli_query($db,$total_pages_sql);
                $total_rows = mysqli_fetch_array($result)[0];
                $total_pages = ceil($total_rows / $no_of_records_per_page);
                $sql = "SELECT product_id, name, description,image,category,MRP,final_cost, created_at FROM products WHERE category='medicine' LIMIT $offset, $no_of_records_per_page";
                $res=$db->query($sql);
                if($res->num_rows>0){
                    $i=0;
